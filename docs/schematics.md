...
npm --cache /private/tmp/aes-npm-cache exec \
  --package ./dist/filippolacagnina-angular-enterprise-starter-0.10.0-alpha.0.tgz \
...
| `tools/schematics/evolutions/design-system/`   | Shared installers and public design-system contracts. |
...
## Public design-system contracts

The npm package exposes two complementary, versioned contracts for trusted build-time tooling.
Both are generated from the same Bootstrap and Tailwind component definitions used by the
installers, so consumers do not need to copy source files, parse internal Schematics modules or run
the CLI in a temporary workspace.

| Contract              | Purpose                                                     | Public subpath          |
| --------------------- | ----------------------------------------------------------- | ----------------------- |
| Design System Catalog | Semantic API, examples and rendering metadata.              | `design-system-catalog` |
| Design System Sources | Runtime Angular files, style requirements and drift hashes. | `design-system-sources` |
...
The catalog includes provider identifiers, selectors, import paths, inputs, outputs and
representative examples. It describes how a tool can present and exercise each component; it does
not contain the component implementation.

Raw JSON is available through the exported
`@filippolacagnina/angular-enterprise-starter/design-system-catalog.json` subpath.
...
The source manifest contains each provider's evolution name, stylesheet strategy, package
requirements and the complete runtime `.ts`, `.html` and `.scss` files generated by the installer.
Paths are provider-relative and normalized. Supplemental specs are intentionally excluded because
preview consumers compile runtime components only.

Raw JSON is available through the exported
`@filippolacagnina/angular-enterprise-starter/design-system-sources.json` subpath.

### Builder synchronization flow

A Builder or another external tool should pin an exact Evolution CLI package version and consume
both contracts from that same package:

```text
pinned Evolution CLI package
  -> validate catalog and source schema versions independently
  -> correlate provider ID, component ID and className
  -> compare source hashes with the preview cache
  -> materialize selected runtime files in an isolated preview workspace
  -> install the provider's declared style dependencies
  -> compile and render the real starter-owned Angular components
```

The catalog remains responsible for examples and component semantics. The source manifest remains
responsible for compilable files and provider styling requirements. The Schematics installers
remain responsible for applying components safely to real product workspaces.

Consumers must not assume that the two independent `schemaVersion` values always advance together.
They should reject unsupported versions and fail when provider IDs, component IDs or class names do
not align.

### Hash and trust contract

- `hashAlgorithm` currently declares `sha256`.
- A component hash covers only normalized, path-sorted runtime file paths and contents.
- A provider hash covers its normalized style requirements and sorted component hashes.
- Supplemental test changes do not invalidate runtime hashes.
- Runtime or style changes invalidate the affected hash and can refresh a Builder cache.

These hashes detect drift and support deterministic caching; they are not signatures and do not
prove package authenticity. Consumers must continue to rely on their package manager, lockfile and
...
npm --cache /private/tmp/aes-npm-cache exec \
  --package ./dist/filippolacagnina-angular-enterprise-starter-0.10.0-alpha.0.tgz \
  -- angular-enterprise-starter evolution --name layout-shell --preview
